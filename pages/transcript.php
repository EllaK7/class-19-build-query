...
if ($order_param == "asc") {
  // ascending 5. sort order (asc)
  $sql_sort_order = "ASC";

  $order_next = "desc";
  $filter_icon = "up";
} else if ($order_param == "desc") {
  // descending 6. sort order (desc)
  $sql_sort_order = "DESC";
...
if ($sql_sort_order && in_array($sort_param, array("course", "term", "year", "credits", "grade"))) {
  ...
  // order by SQL clause 8. order by clause (sort by field ($sql_sort_field) in order ($sql_sort_order)
...
// 3. glue select clause to order clause
$sql_select_query = $sql_select_clause . $sql_order_clause . ";" ;

// query grades table
// 4. query database using the assembled/built query ($sql_select_query)
